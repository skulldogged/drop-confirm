package dev.skulldogged.drop_confirm.screens
...
class PopupScreen(val itemStack: ItemStack, private val entireStack: Boolean) : Screen(ComponentUtils.translatable("gui.drop_confirm")) {
  // Positioning
  ...
  inner class StyledButton(
    ...
    override fun /*$ render_method {*/renderWidget/*$}*/(
      ...
      val color = if (isHovered /*? if >=26.3 {*//*|| isFocused*//*?}*/) hoverColor else baseColor
  ...
  override fun init() {
      ...
      add(
        StyledButton(
          ...
          ComponentUtils.translatable("gui.yes")/*? if <=1.15.2 {*//*.string*//*?}*/,
          {
            ...
            //? if >=26.3 {
            /*minecraft?.gameMode?.dropItem(player, entireStack)
            *///?} else {
    ...
    //? if >=26.3
    /*renderables.filterIsInstance<Button>().forEach { addWidget(it) }*/
  }
